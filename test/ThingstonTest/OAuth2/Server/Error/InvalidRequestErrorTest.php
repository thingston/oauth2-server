...
/**
 * Invalid request error test case.
 *
...
class InvalidRequestErrorTest extends TestCase
{
    ...
    public function testErrorState()
    {
        ...
        $error = new InvalidRequestError($description);

        $this->assertEquals($description, $error->getDescription());
        $this->assertEquals(InvalidRequestError::INVALID_REQUEST_CODE, $error->getCode());
        $this->assertEquals(InvalidRequestError::INVALID_REQUEST_STATUS, $error->getStatus());
        $this->assertEquals(InvalidRequestError::DEFAULT_URI, $error->getUri());
