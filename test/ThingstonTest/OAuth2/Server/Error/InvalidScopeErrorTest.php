...
use Thingston\OAuth2\Server\Error\InvalidScopeError;

/**
 * Invalid scope error test case.
 *
 * @author Pedro Ferreira <pedro@thingston.com>
 */
class InvalidScopeErrorTest extends TestCase
{

    public function testErrorState()
    {
        $description = 'My error description.';
        $error = new InvalidScopeError($description);

        $this->assertEquals($description, $error->getDescription());
        $this->assertEquals(InvalidScopeError::INVALID_SCOPE_CODE, $error->getCode());
        $this->assertEquals(InvalidScopeError::INVALID_SCOPE_STATUS, $error->getStatus());
        $this->assertEquals(InvalidScopeError::DEFAULT_URI, $error->getUri());
