...
/**
 * Unauthorized client error test case.
 *
 * @author Pedro Ferreira <pedro@thingston.com>
 */
class UnauthorizedClientErrorTest extends TestCase
{

    public function testErrorState()
    {
        $description = 'My error description.';
        $error = new UnauthorizedClientError($description);

        $this->assertEquals($description, $error->getDescription());
        $this->assertEquals(UnauthorizedClientError::UNAUTHORIZED_CLIENT_CODE, $error->getCode());
        $this->assertEquals(UnauthorizedClientError::UNAUTHORIZED_CLIENT_STATUS, $error->getStatus());
        $this->assertEquals(UnauthorizedClientError::DEFAULT_URI, $error->getUri());
